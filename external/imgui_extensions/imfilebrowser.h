...
enum ImGuiFileBrowserFlags_ : std::uint32_t
{
    ...
    ImGuiFileBrowserFlags_Open                  = 1 << 12, //displays the string "open" next to the new filename input when enter filename is selected
    ImGuiFileBrowserFlags_SaveAs                = 1 << 13  //displays the string "save as" next to the new filename input when enter filename is selected
};
...
inline void ImGui::FileBrowser::Display()
{
    ...
    if(flags_ & ImGuiFileBrowserFlags_EnterNewFilename)
    {
        ...
        PushItemWidth(-1);
        if (flags_ & ImGuiFileBrowserFlags_SaveAs)
        {
            ImGui::Text("Save as "); ImGui::SameLine();
        }
        if (flags_ & ImGuiFileBrowserFlags_Open)
        {
